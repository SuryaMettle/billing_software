...
function Parties({ onOpenInvoices }) {
  ...
  const creditData = {};
  for (const p of data) {
    const credits = await window.api.getPartyCreditNotes(p.id);
    if (credits?.length > 0) {
  ...
  const loadPartyStats = async () => {
    const invoices = await window.api.getInvoicesWithProfit();
  ...
  const loadSalesStats = async () => {
    const data = await window.api.getSalesStats();
    setSalesStats(data);
  };
  ...
  const handleDelete = async (id) => {
    await window.api.deleteParty(id);
    loadParties();
    loadPartyStats();
  };
