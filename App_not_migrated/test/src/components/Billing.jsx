...
const barcodeRef = useRef(null);
 ...
 const recalculateCart = (nextCart, clearResolved = false) => {
  ...
  const removeItem = (cartItemId) => {
    ...
    setCart(recalculateCart(updated, true));
  };
  ...
  const saveInvoice = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        window.api.getParties().then(setParties);
        window.api.getOffers?.().then((data) => setOffers(Array.isArray(data) ? data : []));
      }
    } catch (err) {
      alert(err.message);
    }
...
const handleDiscountAmount = (cartItemId, amount) => {
  ...
  useEffect(() => {
    window.api.getParties().then(setParties);
    window.api.getLoyaltySettings().then((data) => {
      if (data) setLoyaltySettings(data);
    });
    window.api.getOffers?.().then((data) => {
  ...
  setResolvedConflicts({});
  setCart((prev) => {
    const { cart: newCart, conflicts } = applyOffers(prev, offers, {});
  ...
  useEffect(() => {
    if (selectedParty?.id) {
      window.api.getPartyLoyaltyPoints(selectedParty.id).then((data) => {
...
<div style={{ marginBottom: "16px", padding: "12px", background: "#f0f7ff",
  ...
  {item.appliedOfferId && !item.isFreeItem && !item.isCartDiscountItem && item.offerDiscountAmount > 0 && (
    <span style={{ marginLeft: "8px", background: "#fff3e0", color: "#e65100",
      padding: "2px 6px", borderRadius: "6px", fontSize: "11px", fontWeight: "600",
      cursor: "pointer" }}
      onClick={(e) => {
  e.stopPropagation();
