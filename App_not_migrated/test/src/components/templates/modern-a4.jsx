...
// ─────────────────────────────────────────────
//  MODERN A4 — colored header, rounded cards
// ─────────────────────────────────────────────
...
export function generateModernA4(data, options = {}) {
  ...
  const drawHeader = (pageNum) => {
    ...
    const addressParts = [settings?.address, settings?.city, settings?.state, settings?.pincode]
      .filter(Boolean).join(", ");
    ...
    if (settings?.gstin) {
      doc.setFont("helvetica", "bold");
      doc.text(`GSTIN: ${settings.gstin}`, margin, 25);
    }
    ...
    if (invoice.due_date) {
      ...
      doc.text(`Due Date: ${due}`, pageW - margin, 25, { align: "right" });
    }
    ...
    doc.setFillColor(...sc);
    doc.roundedRect(badgeX, badgeY, 18, 5, 2, 2, "F");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(7);
  ...
  invoice.party_gstin ? `GSTIN: ${invoice.party_gstin}` : null,  // ← add
].filter(Boolean);
  ...
  doc.roundedRect(margin, cursorY, colW, 28, 2, 2, "F");
  doc.setDrawColor(220, 220, 220);
  doc.roundedRect(margin, cursorY, colW, 28, 2, 2, "S");
  ...
  if (hasShipping) {
    ...
    const shipLines = [
      invoice.party_name || "N/A",
      invoice.party_shipping_address,
      [invoice.party_shipping_city, invoice.party_shipping_state, invoice.party_shipping_pincode].filter(Boolean).join(", ") || null,
    ].filter(Boolean);
    doc.setFillColor(249, 249, 249);
    doc.roundedRect(shipX, cursorY, colW, 28, 2, 2, "F");
    doc.setDrawColor(220, 220, 220);
    doc.roundedRect(shipX, cursorY, colW, 28, 2, 2, "S");
    doc.setFont("helvetica", "bold"); doc.setFontSize(8); doc.setTextColor(25, 118, 210);
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      0: { halign: "center", cellWidth: 10 }, 1: { halign: "left", cellWidth: "auto" },
      2: { halign: "center", cellWidth: 22 }, 3: { halign: "center", cellWidth: 12 },
      4: { halign: "right", cellWidth: 20 }, 5: { halign: "right", cellWidth: 22 },
      6: { halign: "center", cellWidth: 20 },
      ...(isSameState ? { 7: { halign: "center", cellWidth: 20 }, 8: { halign: "right", cellWidth: 22 } } : { 7: { halign: "right", cellWidth: 25 } }),
    },
  ...
  const taxCols = isSameState
    ? ["GST Rate", "Taxable Amt (Rs.)", "CGST (Rs.)", "SGST (Rs.)"]
    : ["GST Rate", "Taxable Amt (Rs.)", "IGST (Rs.)"];

  autoTable(doc, {
    startY: cursorY, head: [taxCols], body: taxRows,
    margin: { left: margin, right: pageW / 2 + 4 },
    theme: "grid", tableWidth: contentW / 2 - 4,
  ...
  const totalsData = [
    ["Subtotal (before tax)", `Rs.${subtotal.toFixed(2)}`],
    ...(isSameState
  ...
  if (deliveryCopy || balance > 0) {
    doc.setTextColor(211, 47, 47); doc.setFont("helvetica", "bold");
    doc.text("Balance Due", boxX + 3, paidY + 5);
    doc.text(deliveryCopy ? deliveryBlank : `Rs.${balance.toFixed(2)}`, boxX + boxW - 3, paidY + 5, { align: "right" });
  }
  ...
  doc.text(payInfo, margin, cursorY);
  cursorY += 10;

  doc.setDrawColor(150, 150, 150); doc.setLineWidth(0.4);
  doc.setFont("helvetica", "normal"); doc.setFontSize(8.5); doc.setTextColor(100, 100, 100);
  doc.text("Customer Signature", margin, cursorY);
  doc.line(margin, cursorY + 12, margin + 55, cursorY + 12);
  const sigRightX = pageW - margin - 55;
  doc.text("Authorized Signature", sigRightX, cursorY, { align: "left" });
  doc.line(sigRightX, cursorY + 12, pageW - margin, cursorY + 12);
...
// ── React preview (same layout as before) ──
export function ModernA4Preview({ data, deliveryCopy }) {
  ...
  return (
    <div style={{ width: "210mm", minHeight: "297mm", margin: "24px auto", background: "#fff", padding: "30px 40px", boxShadow: "0 4px 20px rgba(0,0,0,0.12)", fontSize: "13px", color: "#111", boxSizing: "border-box" }}>
      ...
      <div style={{ background: "#f9f9f9", borderRadius: "8px", padding: "12px", marginBottom: "16px", display: "inline-block", minWidth: "200px" }}>
          ...
          {items.map((item, i) => {
            ...
            return (
              <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#f9f9f9" }}>
                ...
                {isSameState ? (
                  <>
                    <td style={{ padding: "8px", textAlign: "center", borderBottom: "1px solid #eee" }}>{rate / 2}%<br />₹{(taxAmt / 2).toFixed(2)}</td>
                    <td style={{ padding: "8px", textAlign: "center", borderBottom: "1px solid #eee" }}>{rate / 2}%<br />₹{(taxAmt / 2).toFixed(2)}</td>
                  </>
                ) : (
                  <td style={{ padding: "8px", textAlign: "center", borderBottom: "1px solid #eee" }}>{rate}%<br />₹{taxAmt.toFixed(2)}</td>
                )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: "36px", paddingTop: "8px" }}>
        <div style={{ textAlign: "left" }}>
          <div style={{ fontSize: "12px", color: "#888", marginBottom: "28px" }}>Customer Signature</div>
          <div style={{ width: "160px", borderTop: "1px solid #aaa" }} />
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: "12px", color: "#888", marginBottom: "28px" }}>Authorized Signature</div>
          <div style={{ width: "160px", borderTop: "1px solid #aaa", paddingTop: "6px" }}>
            {settings?.business_name && <div style={{ fontSize: "11px", fontWeight: "600", color: "#111", textAlign: "right" }}>{settings.business_name}</div>}
          </div>
        </div>
      </div>
