...
// ─────────────────────────────────────────────
//  CLASSIC A4 — formal black & white, GST-ready
// ─────────────────────────────────────────────
...
export function generateClassicA4(data, options = {}) {
  ...
  const drawHeader = (pageNum) => {
    ...
    if (settings?.gstin) {
      doc.setFont("helvetica", "bold");
      doc.text(`GSTIN: ${settings.gstin}`, pageW / 2, 31, { align: "center" });
    }
    ...
    if (invoice.due_date) {
      ...
      doc.text(`Payment Terms: ${(invoice.payment_terms || "").replace("_", " ")}`, margin, 56);
      doc.text(`Due Date: ${due}`, pageW - margin, 56, { align: "right" });
    }
    doc.setLineWidth(0.4);
  ...
  doc.text(invoice.party_name || "N/A", margin + 3, cursorY + 12);
  ...
  if (hasShipping) {
    ...
    doc.setFont("helvetica", "bold"); doc.setFontSize(9.5);
    doc.text(invoice.party_name || "N/A", shipX + 3, cursorY + 12);
    doc.setFont("helvetica", "normal"); doc.setFontSize(8);
  ...
  const tableColumns = isSameState
    ? ["#", "Description of Goods", "HSN/SAC", "Qty", "Rate (Rs.)", "Taxable (Rs.)", "CGST", "SGST", "Total (Rs.)"]
    : ["#", "Description of Goods", "HSN/SAC", "Qty", "Rate (Rs.)", "Taxable (Rs.)", "IGST", "Total (Rs.)"];
  ...
  autoTable(doc, {
    ...
    theme: "grid",
    styles: { fontSize: 8.5, cellPadding: 3, textColor: [0, 0, 0], lineColor: [0, 0, 0], lineWidth: 0.3, valign: "middle" },
    headStyles: { fillColor: [255, 255, 255], textColor: [0, 0, 0], fontStyle: "bold", halign: "center", lineColor: [0, 0, 0], lineWidth: 0.4 },
    columnStyles: {
  ...
  const taxCols = isSameState
    ? ["GST Rate", "Taxable (Rs.)", "CGST (Rs.)", "SGST (Rs.)"]
    : ["GST Rate", "Taxable (Rs.)", "IGST (Rs.)"];

  autoTable(doc, {
    startY: cursorY, head: [taxCols], body: taxRows,
    margin: { left: margin, right: pageW / 2 + 4 }, tableWidth: contentW / 2 - 4,
    theme: "grid",
    styles: { fontSize: 8, cellPadding: 2.5, textColor: [0, 0, 0], lineColor: [0, 0, 0], lineWidth: 0.3 },
    headStyles: { fillColor: [240, 240, 240], textColor: [0, 0, 0], fontStyle: "bold", halign: "center", lineColor: [0, 0, 0] },
  ...
  doc.setDrawColor(0); doc.setLineWidth(0.4);
  doc.rect(boxX, gtY, boxW, 9);
  doc.setFont("helvetica", "bold"); doc.setFontSize(10); doc.setTextColor(0);
  doc.text("GRAND TOTAL", boxX + 3, gtY + 6);
  doc.text(`Rs.${total.toFixed(2)}`, boxX + boxW - 3, gtY + 6, { align: "right" });
  ...
  if (deliveryCopy || balance > 0) {
    doc.setFont("helvetica", "bold");
    doc.text(`Balance Due: ${deliveryCopy ? deliveryBlank : `Rs.${balance.toFixed(2)}`}`, boxX + 3, paidY + 5);
  }

  cursorY = Math.max(taxEndY, paidY + 12) + 10;

  // Bottom border + signature
  doc.setDrawColor(0); doc.setLineWidth(0.4);
  ...
  doc.line(margin, cursorY + 14, margin + 55, cursorY + 14);
  const sigX = pageW - margin - 55;
  doc.text("Authorised Signatory", sigX, cursorY);
  doc.line(sigX, cursorY + 14, pageW - margin, cursorY + 14);
  if (settings?.business_name) {
    doc.setFont("helvetica", "bold");
    doc.text(settings.business_name, pageW - margin, cursorY + 19, { align: "right" });
  }

  doc.setDrawColor(0); doc.setLineWidth(0.4);
  doc.line(margin, cursorY + 24, pageW - margin, cursorY + 24);
  doc.setLineWidth(1.2);
  doc.line(margin, cursorY + 26, pageW - margin, cursorY + 26);
...
export function ClassicA4Preview({ data, deliveryCopy }) {
  ...
  return (
    <div style={{ width: "210mm", minHeight: "297mm", margin: "24px auto", background: "#fff", padding: "20px 30px", boxShadow: "0 4px 20px rgba(0,0,0,0.12)", fontSize: "12px", color: "#000", boxSizing: "border-box", fontFamily: "serif" }}>
      ...
      <div style={{ borderTop: "1px solid #000", borderBottom: "3px solid #000", textAlign: "center", padding: "4px 0", fontWeight: "700", fontSize: "13px", letterSpacing: "2px" }}>
      ...
      <div style={{ border: border, padding: "8px", marginBottom: "12px", display: "inline-block", minWidth: "200px" }}>
          ...
          {items.map((item, i) => {
            ...
            return (
              <tr key={i}>
                ...
                {isSameState ? (
                  <><td style={{ padding: "6px", textAlign: "center", border }}>{rate / 2}%<br />₹{(taxAmt / 2).toFixed(2)}</td>
                  <td style={{ padding: "6px", textAlign: "center", border }}>{rate / 2}%<br />₹{(taxAmt / 2).toFixed(2)}</td></>
                ) : (
                  <td style={{ padding: "6px", textAlign: "center", border }}>{rate}%<br />₹{taxAmt.toFixed(2)}</td>
                )}
