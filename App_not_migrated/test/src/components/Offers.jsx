...
export default function Offers({ products = [], categories: categoriesFromApp = [] }) {
  ...
  const loadOffers = async () => {
    try {
      const data = await window.api.getAllOffers?.();
  ...
  useEffect(() => {
    ...
    window.api.getCategories?.().then((data) => {
...
setLoading(true);
    ...
    try {
      const res = await window.api.createOffer({
        ...form,
  ...
  const handleToggle = async (offer) => {
    ...
    await window.api.toggleOffer?.({ id: offer.id, active: newActive });
    await loadOffers();
  };
