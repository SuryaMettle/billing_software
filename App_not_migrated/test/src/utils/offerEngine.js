...
function applyFlatDiscount(cart, offer) {
  ...
  // ── FIX: use actual item totals (already reduced by offer discounts) ──
  const cartTotal = cart
    .filter((i) => !i.isCartDiscountItem && !i.isFreeItem)
    .reduce((s, i) => s + Number(i.total || 0), 0);
  ...
  const discountRow = createCartDiscountItem({
    offerId: id,
    offerName: name,
    flatAmount: Math.min(flatAmount, cartTotal),
  });

  return [...cart, discountRow];
}
