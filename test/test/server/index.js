...
import db, { backupDatabase } from "../electron/database.js";
...
app.get("/api/export-excel", adminOnly, asyncRoute(async (req, res) => {
  const tables = ["products", "parties", "invoices", "invoice_items", "invoice_payments",
    "purchase_invoices", "purchase_invoice_items", "purchase_payments", "offers",
    "sales_returns", "sales_return_items", "purchase_returns", "purchase_return_items",
    "settings", "users", "loyalty_ledger", "credit_notes", "return_refunds"];
  ...
  try {
      ...
      ? rows.map(row => {
          ...
          for (const [key, val] of Object.entries(row)) {
            if (typeof val === "string" && val.length > 32000) {
              clean[key] = val.slice(0, 32000) + "...[truncated]";
            } else {
              clean[key] = val;
            }
          }
    ...
    console.error(`Export error for ${table}:`, e.message);
  }
}
