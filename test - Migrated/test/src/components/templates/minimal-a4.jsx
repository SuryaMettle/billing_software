...
export function generateMinimalA4(data, options = {}) {
  ...
  const drawHeader = (pageNum) => {
    ...
    if (invoice.due_date) { const due = new Date(invoice.due_date).toLocaleDateString("en-IN"); doc.text(`Due: ${due}`, pageW - margin, 29, { align: "right" }); }
    doc.setDrawColor(...LIGHT); doc.setLineWidth(0.5); doc.line(margin, 36, pageW - margin, 36);
    doc.setFontSize(7.5); doc.setTextColor(...LIGHT);
  ...
  if (hasParty) {
    ...
    doc.text(invoice.party_name, margin, cursorY);
    cursorY += 5;
  ...
  try {
    doc.addImage(invoice.party_logo, "PNG", margin, cursorY, 28, 10, "", "FAST");
    cursorY += 12;
  } catch(e) {}
}
    cursorY += 4;
  ...
  const tableColumns = isSameState ? ["#", "Item", "HSN", "Qty", "Price", "Taxable", "CGST", "SGST", "Total"] : ["#", "Item", "HSN", "Qty", "Price", "Taxable", "IGST", "Total"];
  ...
  autoTable(doc, { startY: cursorY, head: [tableColumns], body: tableRows, margin: { left: margin, right: margin }, theme: "plain", styles: { fontSize: 8.5, cellPadding: 3, textColor: [40, 40, 40], valign: "middle" }, headStyles: { fontStyle: "bold", textColor: [...ACCENT], fontSize: 7.5, cellPadding: { top: 2, bottom: 4 } }, columnStyles: { 0: { halign: "center", cellWidth: 8 }, 1: { halign: "left", cellWidth: "auto" }, 2: { halign: "center", cellWidth: 20 }, 3: { halign: "center", cellWidth: 12 }, 4: { halign: "right", cellWidth: 20 }, 5: { halign: "right", cellWidth: 22 }, 6: { halign: "center", cellWidth: 20 }, ...(isSameState ? { 7: { halign: "center", cellWidth: 20 }, 8: { halign: "right", cellWidth: 22 } } : { 7: { halign: "right", cellWidth: 24 } }) }, alternateRowStyles: { fillColor: [250, 250, 250] }, didDrawPage: () => { const p = doc.internal.getCurrentPageInfo().pageNumber; if (p > 1) drawHeader(p); }, showHead: "everyPage" });

  cursorY = doc.lastAutoTable.finalY + 2;
  doc.setDrawColor(...LIGHT); doc.setLineWidth(0.3); doc.line(margin, cursorY, pageW - margin, cursorY); cursorY += 6;
  ...
  const taxCols = isSameState ? ["GST Rate", "Taxable", "CGST", "SGST"] : ["GST Rate", "Taxable", "IGST"];
  autoTable(doc, { startY: cursorY, head: [taxCols], body: taxRows, margin: { left: margin, right: pageW / 2 + 4 }, tableWidth: contentW / 2 - 4, theme: "plain", styles: { fontSize: 7.5, cellPadding: 2, textColor: [...ACCENT] }, headStyles: { fontStyle: "bold", textColor: [...ACCENT] }, columnStyles: { 0: { halign: "left" }, 1: { halign: "right" }, 2: { halign: "right" }, 3: { halign: "right" } }, didDrawPage: () => {} });

  const taxEndY = doc.lastAutoTable.finalY;
  const boxX = pageW / 2 + 4; const boxW = contentW / 2 - 4;
  const totalsData = [["Subtotal", `Rs.${subtotal.toFixed(2)}`], ...(isSameState ? [["CGST", `Rs.${(totalTax / 2).toFixed(2)}`], ["SGST", `Rs.${(totalTax / 2).toFixed(2)}`]] : [["IGST", `Rs.${totalTax.toFixed(2)}`]])];
  autoTable(doc, { startY: cursorY, body: totalsData, margin: { left: boxX, right: margin }, tableWidth: boxW, theme: "plain", styles: { fontSize: 8.5, cellPadding: 2.5, textColor: [60, 60, 60] }, columnStyles: { 0: { halign: "left" }, 1: { halign: "right" } }, didDrawPage: () => {} });

  const gtY = doc.lastAutoTable.finalY + 2;
  doc.setDrawColor(40, 40, 40); doc.setLineWidth(0.5); doc.line(boxX, gtY, boxX + boxW, gtY);
  doc.setFont("helvetica", "bold"); doc.setFontSize(11); doc.setTextColor(20, 20, 20);
  doc.text("Total", boxX, gtY + 6); doc.text(`Rs.${total.toFixed(2)}`, boxX + boxW, gtY + 6, { align: "right" });
  doc.setDrawColor(...LIGHT); doc.setLineWidth(0.3); doc.line(boxX, gtY + 8, boxX + boxW, gtY + 8);
  const paidY = gtY + 13;
  doc.setFont("helvetica", "normal"); doc.setFontSize(8.5); doc.setTextColor(...ACCENT);
  doc.text("Paid", boxX, paidY); doc.text(deliveryCopy ? deliveryBlank : `Rs.${paid.toFixed(2)}`, boxX + boxW, paidY, { align: "right" });
  if (deliveryCopy || balance > 0) { doc.setFont("helvetica", "bold"); doc.setTextColor(180, 0, 0); doc.text("Balance Due", boxX, paidY + 5); doc.text(deliveryCopy ? deliveryBlank : `Rs.${balance.toFixed(2)}`, boxX + boxW, paidY + 5, { align: "right" }); }

  cursorY = Math.max(taxEndY, paidY + 10) + 14;
  doc.setDrawColor(...LIGHT); doc.setLineWidth(0.3); doc.line(margin, cursorY, pageW - margin, cursorY); cursorY += 8;
  doc.setFont("helvetica", "normal"); doc.setFontSize(8); doc.setTextColor(...ACCENT);
  doc.text(`Payment: ${invoice.payment_mode || "-"}`, margin, cursorY); cursorY += 14;
  doc.text("Customer Signature", margin, cursorY); doc.setDrawColor(...LIGHT); doc.line(margin, cursorY + 12, margin + 50, cursorY + 12);
  const sigX = pageW - margin - 50;
  doc.text("Authorised by", sigX, cursorY); doc.line(sigX, cursorY + 12, pageW - margin, cursorY + 12);
  if (settings?.business_name) { doc.setFont("helvetica", "bold"); doc.setFontSize(8); doc.setTextColor(20, 20, 20); doc.text(settings.business_name, pageW - margin, cursorY + 17, { align: "right" }); }
          ...
          {items.map((item, i) => {
            ...
            return (
              <tr key={i} style={{ borderBottom: `1px solid ${light}`, background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                ...
                {isSameState ? (<><td style={{ padding: "8px 4px", textAlign: "center", color: accent }}>{rate / 2}%<br />₹{(taxAmt / 2).toFixed(2)}</td><td style={{ padding: "8px 4px", textAlign: "center", color: accent }}>{rate / 2}%<br />₹{(taxAmt / 2).toFixed(2)}</td></>) : (<td style={{ padding: "8px 4px", textAlign: "center", color: accent }}>{rate}%<br />₹{taxAmt.toFixed(2)}</td>)}
